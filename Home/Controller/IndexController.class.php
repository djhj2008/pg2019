...
class IndexController extends Controller {
	...
	public function pushnewsubV30(){
		...
		$BTSN_LEN  = 10;//¨ª3¡À¨¤10??1¨¤¨¤D¨ª,2-41¨²?¨°¡À¨¤??,5-10??¨®¨°¡À¨¤??
		$BDSN_LEN  = 4;//BS¡Á?¡¤?3¡è?¨¨
		$BSN_LEN  = $BTSN_LEN+$BDSN_LEN;//BS¡Á?¡¤?3¡è?¨¨
    ...
    $BRSSI_SIGN_LEN = 1;
		...
		$COUNT_LEN =2; //data¦Ì?¨¬?¨ºy
		$CSN_LEN  =4;//¨¦¨¨¡À?¡Á?¡¤?3¡è?¨¨
		$SIGN_LEN =1;//D?o?
		$CVS_LEN =1;//client version
		$STATE_LEN  =1;//state
		$DELAY_LEN  =1;//delay
		$VAILD_LEN  =1;//¨®DD¡ì?¦Ì??¨ºy
		
		$SENS_LEN  =1;//¨®DD¡ì?¦Ì??¨ºy
		
		$VALUE_LEN = 10;//data?D????3¡è?¨¨
		$COUNT_VALUE = 4;

		$DATA_LEN = ($CSN_LEN+$SIGN_LEN+$CVS_LEN+$STATE_LEN+$DELAY_LEN+$VAILD_LEN+$SENS_LEN)*2+$VALUE_LEN*$COUNT_VALUE; //¨°?¨¬?data3¡è?¨¨
		
		//var_dump($DATA_LEN);
		
		$CRC_LEN  = 4;//D¡ê?¨¦??
		$post      =file_get_contents('php://input');//¡Á£¤¨¨??¨²¨¨Y
    $strarr    =unpack("H*", $post);//unpack() o¡¥¨ºy¡ä¨®?t????¡Á?¡¤?¡ä???¨ºy?Y??DD?a¡ã¨¹?¡ê
    $str       =implode("", $strarr);
    ...
    $count     =substr($str,$CDATA_START*2,$COUNT_LEN*2);//2?a?a¡ã¨¹o¨®¦Ì?¡À?¨ºy
    $count	   =hexdec($count);//¡ä¨®¨º?¨¢¨´????¡Áa¨º?????
    ...
    //?¡ä?a¡ã¨¹¡À¨¨??
    $len=strlen($str);
    $crc=substr($str,$len-$CRC_LEN*2);//¨º?¦Ì?¡¤¡é¨¤¡ä¦Ì?crc
    //var_dump($crc);
    $crc=hexdec($crc);
    //var_dump($crc);
    ...
    $len = strlen($str);
	    ...
	    for($i=0 ; $i < $count ; $i++){
	    	...
	    	//var_dump($snstr);
	    	$snint = hexdec($snstr)&0x1fff;;	//¡ä¨®¨º?¨¢¨´????¡Áa¨º?????
	    	...
	    	if($cvs>3){
	    		...
	    		$tempstr	 =	substr($data, $i*$DATA_LEN+($CSN_LEN+$SIGN_LEN+$CVS_LEN+$STATE_LEN+$DELAY_LEN+$SENS_LEN+$VAILD_LEN)*2,$VALUE_LEN*$COUNT_VALUE);//temp1¨º?¨¢¨´????¡Á?¡¤?
	    		...
	    		$tempstr   = substr($data, $i*$DATA_LEN+($CSN_LEN+$SIGN_LEN+$CVS_LEN+$STATE_LEN+$DELAY_LEN+$VAILD_LEN)*2,$VALUE_LEN*$COUNT_VALUE);//temp1¨º?¨¢¨´????¡Á?¡¤?
	    	}
	    	$vaild = hexdec($vaildstr);
	    	if($type>0){
	    		$type=1;
  ...
  public function parsedata($data,$psnid,$sid,$interval){
    	...
    	if($cvs>3){
    		...
    		$tempstr	 =	substr($data, ($CSN_LEN+$SIGN_LEN+$CVS_LEN+$STATE_LEN+$DELAY_LEN+$SENS_LEN+$VAILD_LEN)*2,$VALUE_LEN*$COUNT_VALUE);//temp1¨º?¨¢¨´????¡Á?¡¤?
				$sens=0-hexdec($sensstr);
    	}else{
				$sens=0;
				$vaildstr  = substr($data, ($CSN_LEN+$SIGN_LEN+$CVS_LEN+$STATE_LEN+$DELAY_LEN)*2,$VAILD_LEN*2);
    		$tempstr   = substr($data, ($CSN_LEN+$SIGN_LEN+$CVS_LEN+$STATE_LEN+$DELAY_LEN+$VAILD_LEN)*2,$VALUE_LEN*$COUNT_VALUE);//temp1¨º?¨¢¨´????¡Á?¡¤?
    	}
    	$vaild = hexdec($vaildstr);
    	if($type>0){
    		$type=1;
