...
class DatapushTest2Controller extends Controller {
    ...
    $str       =implode("", $strarr);
		...
		$str = "3230323030343238313930303130313130383637353633303000008001055000000000000000000002000080063d530400015352255702ffffffffffffffffffffffffffffffff000080073e530400015752255402ffffffffffffffffffffffffffffffff00002a29";
    $sndir = substr($str, ($TIME_LEN+$BTSN_LEN)*2,$BDSN_LEN*2);
    $sn_footer = hexdec($sndir)&0x1fff;
    $sn_header = hexdec($sndir)>>13;
    $logbase="lora_log/backupV3030/";
    $logerr="lora_log/errorV3030/";
    $logreq="lora_log/reqV3030/";
    ob_clean();
    ...
    $user2=D('taccess');
  	...
  	foreach($re_devs2 as $redev){
		...
		$body=$header.$delay_time.$rate.$log_flag.$change_str.$footer.$devres_str;
