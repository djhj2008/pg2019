...
class DatapushTest2Controller extends Controller {
    ...
    $logbase="lora_backup230/";
    $logerr="lora_error230/";
    $logreq="lora_req230/";
    ...
    $len = strlen($str);
  	...
  	foreach($re_devs2 as $redev){
		...
		if($crc==$sum){
			$year=date('Y')+0;
			$header="OK1".$year.date('mdHis');
		}else{
			$year=date('Y')+0;
			$header="OK2".$year.date('mdHis');
		}
	...
	public function pushnewsubV47()
	{
	    ...
	    $sn_footer = hexdec($sndir) & 0x1fff;
	    $sn_header = hexdec($sndir) >> 13;
	    $logbase = "lora_backup247/";
	    $logerror = "lora_error247/";
	    $logreq = "lora_req247/";
	...
	public function pushnewsubV48()
	{
      ...
      if(count($ch_list_buf)>0){
	    ...
	    }
	    {
			...
			$result = $header.$changedev_str.$blacklist_str;
			//$result=$header;
			$sum=0;
	    $len = strlen($result);
			for($i=0 ; $i < $len;$i++)
			{
				$value = ord(substr($result, $i,1));
				$sum+=$value;
			}
			$sum=$sum&0xffffffff;
			//var_dump($sum);
			$sum=dechex($sum);
			//var_dump($sum);
			$sum=str_pad($sum,8,'0',STR_PAD_LEFT);
