...
class ProductController extends HomeController {
  ...
  public function delrfid(){
	...
	public function scanfactory(){
  	...
  	$pre_time = $cur_time-$delay*$count+$start_time-$delay;
  	$pre2_time = $cur_time-$delay*$count+$start_time-$delay*2;
  	$pre3_time = $cur_time-$delay*$count+$start_time-$delay*3;
  	...
  	$wheredev['devid']=array('in',$devidlist);
		...
		foreach($alldevs as $dev){
			$flaglist[$dev['devid']]=$dev['flag'];
			$rf_list[$dev['devid']]=$dev['rid'];
		}
  	...
  	$accSelect1=M($mydb)->where(array('psn'=>$psn,'time'=>$first_time))->where($wheredev)->order('devid asc')->select();
		...
		foreach($devSelect as $key=>$dev){
			$devid=$dev['devid'];
			$devSelect[$key]['flag']=$flaglist[$devid];
			$devSelect[$key]['rid']=$rf_list[$devid];
		}
	...
	public function factorytempeor(){
  	...
  	$cur_time = (int)($cur_time/$delay)*$delay;
  	$first_time = $cur_time-$delay*$count+$start_time;
  	$last_time = $cur_time-$delay*$count+$start_time-$delay;	
	...
	public function devselect(){
		...
		$devs=M('device')->where(array('psnid'=>$psnid))->order('devid asc')->select();
		foreach($devs as $dev){
			$rf_list[$dev['devid']]=$dev['rid'];
		}
		...
		foreach($devSelect as $key=>$devsl){
			$devSelect[$key]['rid']=$rf_list[$devsl['devid']];
		}
	...
	public function scandevlow(){
    	...
    	//dump(count($devlist));
    	//exit;
			$accSelect=M('access_base')->where($wheredev)->where('time >='.$last_time.' and time <='.$first_time)->where(array('psn'=>$psnid))->order('time desc')->select();
			//dump(count($accSelect));
			foreach($devlist as $dev){
				$devid = $dev['devid'];
				$acc_size=0;
		...
		}
    {
        ...
        if($selectSql=M($mydb)->group('time')->where('devid ='.$id.' and psn= '.$psn.' and time >= '.$start_time.' and time <= '.$end_time)->order('id desc')->select()){
            ...
            echo "<script type='text/javascript'>alert('ûԐ�ꑯ���ẻ.');distory.back();</script>";
        }
    }
		$this->display();
	}
	...
	public function getdevnone(){
		$psn=$_GET['psn'];
		$start=(int)$_GET['start'];
		$count=(int)$_GET['count'];
		$devlist=M('device')->where(array('psn'=>$psn))->select();
		if($count==count($devlist)){
			echo 'all recv.'.$count.'<br>';
			//exit;
		}
		for($i=$start;$i<($start+$count);$i++){
			...
			if($find_dev===false){
				echo 'not recv dev:'.$i.'<br>';
			}
		}
		exit;
	}
	
	public function devaddnew(){
			$productno=$_GET['productno'];
			$psn=$_GET['psn'];
			$productlog=M('productlog')->where(array('productno'=>$productno,'psnid'=>$psn))->find();
